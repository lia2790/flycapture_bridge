...
MarkerDetector::MarkerDetector() {
    _thresMethod = ADPT_THRES/*CANNY*/;
    _thresParam1 = _thresParam2 = 7;
    _cornerMethod = LINES/*HARRIS*/;
    _useLockedCorners = false;
    //_cornerMethod=SUBPIX;
    _thresParam1_range = 0;
    _markerWarpSize = 56;
    _speed = 0;
    markerIdDetector_ptrfunc = mid::FiducidalMarkers::detect;
    _minSize = 0.02;
    _maxSize = 0.5;
...
void MarkerDetector::detect(const cv::Mat &input, vector< Marker > &detectedMarkers, Mat camMatrix, Mat distCoeff, float markerSizeMeters,
    ...
    {    cv::cvtColor(input, grey, CV_BGR2GRAY); /*cv::imshow("imageIntoDetect_row107",input);//work*/ }
    else
        grey = input;
	...
	//cv::imshow("greyImage_row116",grey); work
    cv::Mat imgToBeThresHolded = grey;
	//cv::imshow("imgDetect_row118",imgToBeThresHolded);
...
//______________________________________
//
// STEP 2) FIND RECTANGLES INTO IMAGE
//______________________________________
//
...
//
// STEP 4) REFINE THE CORNER LOCATION IF DESIRED
//___________________________________________________________-
//
    ...
    if (detectedMarkers.size() > 0 && _cornerMethod != NONE && _cornerMethod != LINES) {
        ...
        if (_useLockedCorners)//default is false
...
//
// STEP 6) FINALLY CALCULATE THE POSE ESTIMATE
//_____________________________________________
...
void MarkerDetector::thresHold(int method, const Mat &grey, Mat &out, double param1, double param2) throw(cv::Exception) {
    ...
    case ADPT_THRES: // currently, this is the best method
        ...
        cv::adaptiveThreshold(grey, out, 255, ADAPTIVE_THRESH_MEAN_C, THRESH_BINARY_INV, param1, param2); // THE BEST
        break;
    case CANNY: {
